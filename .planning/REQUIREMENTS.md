...
- [ ] **BFF-02**: The BFF **injects each service's auth server-side** — gateway `X-Tenant-Id`/`X-User-Id` (and optional `X-Project-Id`/`X-Session-Id`) from operator context, flowd `Authorization: Bearer` from server config — and **never exposes the flowd token to the browser**; inbound client-set scope headers are stripped before server-set.
