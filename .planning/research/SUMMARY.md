...
The **#1 risk** is SSE buffering: every layer between the backend and the browser (the BFF's own framework, nginx/LB, gzip middleware) buffers by default and silently degrades the console's core value into "spinner, then everything at once." This must be de-risked first: prove unbuffered streaming through a *real* proxy with compression on as an explicit acceptance gate (`curl -N` + browser), with a BFF heartbeat (`: ping`) to survive idle timeouts and an explicit terminal `done` event so clients stop reconnecting. The other foundational invariants are an auth boundary that **never leaks the flowd bearer to the browser** and **strips inbound client scope headers** (setting gateway `X-Tenant-Id`/`X-User-Id` server-side), plus **allowlisted mapped routes** (no SSRF/pass-through). The recommended build order is vertical slices — foundation → Memory (REST-only, de-risks auth before SSE) → Flow (introduces SSE) → Chat (reuses SSE) → health/errors → compose deploy.
...
- **Service health indicators** — polled `/healthz`/`/readyz` per service; colored dot + last-checked.
...
The canonical **BFF / single-origin reverse-proxy** topology: one SPA + one BFF process under one origin, the BFF fanning out to three upstreams and injecting each one's distinct auth server-side. The only non-standard wrinkle is SSE-over-POST on two of three upstreams, which constrains both runtime and client. Recommended layout (Go BFF + embedded SPA): `cmd/console`, `internal/proxy/{memory,flow,chat,sse}.go` (one director per upstream — explicit, auditable auth boundaries), `internal/router` + `health` + `config`, and a `web/` SPA (`features/{memory,flow,chat}`, `lib/api.ts` REST + `lib/sse.ts` SSE-over-POST kept separate).
...
3. **Static asset server + SPA shell** — `go:embed` bundle, SPA `index.html` fallback; shell owns nav + operator-context provider that feeds gateway headers.
...
2. **Idle/proxy timeouts kill long streams** — quiet runs severed at ~30/60s. Avoid: BFF **heartbeat** (`: keepalive` every 15-20s) as part of the streaming primitive; disable global WriteTimeout on stream routes.
3. **Leaking the flowd bearer (and gateway scope) to the browser** — collapses the whole security model. Avoid: token in BFF env only, injected on the upstream hop; browser→BFF auth via same-origin cookie; grep gate on bundle/responses/logs; never put the token in an SSE URL.
4. **Confused-deputy / SSRF + header-scope spoofing** — gateway trusts `X-Tenant-Id`/`X-User-Id` with no other auth. Avoid: **allowlist explicit mapped routes** (never pass-through to a client-supplied target); **strip inbound scope headers** and set them server-side from operator context; pin upstream hosts.
...
**Delivers:** Flow director (inject `Bearer FLOWD_TOKEN`); **SSE route wired unbuffered + heartbeat + ctx cancellation**; `@microsoft/fetch-event-source` client; **shared SSE event-timeline renderer**; flow CRUD; live run viewer; run history + `/events` browse + `/replay`; invalidate runs list on completion.
...
**Rationale:** Single long-lived process (Go binary w/ embedded SPA) as a compose service in the umbrella stack — **not serverless/edge** (execution limits + buffering break long streams).
**Delivers:** Compose service alongside existing stack; ensure no fronting proxy buffers `/stream/*`; document required LB idle-timeout.
...
- **Phase 3 (Flow/SSE):** Highest-risk phase. Needs concrete BFF SSE-flush hardening + heartbeat + replay/dedupe design verified against the *actual* deploy proxy. Confirm whether flowd/chat emit their own heartbeats and whether flowd honors any resume — `/gsd:plan-phase --research-phase` recommended.
...
### Gaps to Address
- **Backend SSE behavior unknown:** Whether flowd/chat already emit heartbeats, and whether flowd supports any `Last-Event-ID`-style resume (research says it uses a *separate* replay endpoint). → Verify during Phase 3 planning; default to BFF-injected heartbeat + `/events`-backfill + dedupe.
...
- **Operator auth on the BFF itself:** PROJECT.md says "BFF holds operator-side credentials" but the mechanism (same-origin session cookie) is unspecified. → Decide in Phase 1; same-origin cookie is the recommended pattern (also required because fetch streams need browser→BFF auth without exposing the flowd token).
